...
export default function SchemaToolbar() {
  ...
  const markAsSaved = useSchemaStore((state) => state.markAsSaved);
  ...
  const handleSave = async () => {
    ...
    if (!confirm("Are you sure you want to save the current schema?")) {
      return;
    }
      ...
      <div className="absolute top-4 left-4 z-10 bg-white p-3 rounded-lg shadow-xl border border-secondary-300 space-y-1.5 w-52">
        ...
        <div className="space-y-1.5">
          ...
          <div className="border-t border-secondary-200 pt-2 mt-2">
            <h3 className="text-xs font-semibold text-secondary-600 mb-1.5 uppercase tracking-wide">
              JSON Model Schema
            </h3>
          ...
          <div className="border-t border-secondary-200 pt-1.5 mt-1.5 space-y-1">
            <button
              ...
              title={hasUnsavedChanges ? "Save project" : "No unsaved changes"}
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
              </svg>
              {isSaving ? "Saving..." : hasUnsavedChanges ? (isEditMode ? "Save" : "Save Project") : "Saved"}
            </button>
